...
import { h as alertController } from './overlays-4199b0b7.js';
...
function sayHello() { return Math.random() < 0.5 ? 'Hello' : 'Hola'; }
...
(function (NakamaAuthenticationMethod) {
  NakamaAuthenticationMethod["Apple"] = "Apple";
  NakamaAuthenticationMethod["Custom"] = "Custom";
  NakamaAuthenticationMethod["Device"] = "Device";
  NakamaAuthenticationMethod["Email"] = "Email";
  NakamaAuthenticationMethod["Facebook"] = "Facebook";
  NakamaAuthenticationMethod["FacebookInstantGame"] = "FacebookInstantGame";
  NakamaAuthenticationMethod["GameCenter"] = "GameCenter";
  NakamaAuthenticationMethod["Google"] = "Google";
  NakamaAuthenticationMethod["Steam"] = "Steam";
})(NakamaAuthenticationMethod || (NakamaAuthenticationMethod = {}));
;
class NakamaSessionManager {
  constructor() { }
  get host() { return NAKAMA_HOST; }
  get port() { return NAKAMA_PORT; }
  get ssl() { return NAKAMA_SSL; }
  get client() { return NAKAMA_CLIENT; }
  get authentication() {
    var value = this['_authentication'];
    if (!value) {
      var key = NAKAMA_AUTHENTICATION_KEY;
  ...
  set authentication(value) {
    ...
    if (json != localStorage.getItem(key)) {
      // authentication is changing, discard session
      this.session = null;
      this['_authentication'] = value;
  ...
  get session() {
    var value = this['_session'];
    if (!value) {
      var key = NAKAMA_SESSION_TOKEN_KEY;
      var token = localStorage.getItem(key);
      if (token) {
        value = Session.restore(token);
        this['_session'] = value;
      }
    }
    return value;
  ...
  set session(value) {
    var key = NAKAMA_SESSION_TOKEN_KEY;
    this['_session'] = value;
    var token = value === null || value === void 0 ? void 0 : value.token;
    if (token) {
  ...
  async authenticateApple(token, create, username, vars, options) {
    this.authentication = {
      method: NakamaAuthenticationMethod.Apple,
      token: token,
      create: create !== null && create !== void 0 ? create : true,
      username: username,
      vars: vars,
      options: options,
    };
  ...
  async authenticateCustom(id, create, username, vars, options) {
    this.authentication = {
      method: NakamaAuthenticationMethod.Custom,
      id: id,
      create: create !== null && create !== void 0 ? create : true,
      username: username,
      vars: vars,
  ...
  async authenticateDevice(id, vars) {
    this.authentication = {
      method: NakamaAuthenticationMethod.Device,
      id: id,
      vars: vars,
    };
    return await this.authenticate();
  }
  async authenticateEmail(email, password, vars) {
    this.authentication = {
      method: NakamaAuthenticationMethod.Email,
      email: email,
      password: password,
      vars: vars,
    };
  ...
  async authenticateFacebook(token, create, username, sync, vars, options) {
    this.authentication = {
      method: NakamaAuthenticationMethod.Facebook,
      token: token,
      create: create !== null && create !== void 0 ? create : true,
      username: username,
      sync: sync,
  ...
  async authenticateFacebookInstantGame(signedPlayerInfo, create, username, vars, options) {
    this.authentication = {
      method: NakamaAuthenticationMethod.FacebookInstantGame,
  ...
  async authenticateGameCenter(token, vars) {
    this.authentication = {
      method: NakamaAuthenticationMethod.GameCenter,
      token: token,
      vars: vars,
    };
    return await this.authenticate();
  }
  async authenticateGoogle(token, create, username, vars, options) {
    this.authentication = {
      method: NakamaAuthenticationMethod.Google,
      token: token,
      create: create !== null && create !== void 0 ? create : true,
      username: username,
      vars: vars,
  ...
  async authenticateSteam(token, vars) {
    this.authentication = {
      method: NakamaAuthenticationMethod.Steam,
      token: token,
      vars: vars,
    };
    return await this.authenticate();
  ...
  async authenticate() {
    ...
    if (auth) {
      try {
        switch (auth.method) {
          case NakamaAuthenticationMethod.Apple:
            return this.session = await NAKAMA_CLIENT.authenticateApple(auth.token, auth.create, auth.username, auth.vars, auth.options);
          case NakamaAuthenticationMethod.Custom:
            return this.session = await NAKAMA_CLIENT.authenticateCustom(auth.id, auth.create, auth.username, auth.vars, auth.options);
          case NakamaAuthenticationMethod.Device:
            return this.session = await NAKAMA_CLIENT.authenticateDevice(auth.id);
          case NakamaAuthenticationMethod.Email:
            return this.session = await NAKAMA_CLIENT.authenticateEmail(auth.email, auth.password);
          case NakamaAuthenticationMethod.Facebook:
            return this.session = await NAKAMA_CLIENT.authenticateFacebook(auth.token, auth.create, auth.username, auth.sync, auth.vars, auth.options);
          case NakamaAuthenticationMethod.FacebookInstantGame:
            return this.session = await NAKAMA_CLIENT.authenticateFacebookInstantGame(auth.signedPlayerInfo, auth.create, auth.username, auth.vars, auth.options);
          case NakamaAuthenticationMethod.GameCenter:
            return this.session = await NAKAMA_CLIENT.authenticateGameCenter(auth.token, auth.vars);
          case NakamaAuthenticationMethod.Google:
            return this.session = await NAKAMA_CLIENT.authenticateGoogle(auth.token, auth.create, auth.username, auth.vars, auth.options);
          case NakamaAuthenticationMethod.Steam:
            return this.session = await NAKAMA_CLIENT.authenticateSteam(auth.token, auth.vars);
        }
      }
      catch (e) {
  ...
  async acquireSession() {
    ...
    if (!session || session.isexpired(nowSeconds + 10)) {
      this.session = session = await this.authenticate();
    }
    return session;
  }
  async addFriends(ids, usernames) {
    return NAKAMA_CLIENT.addFriends(await this.acquireSession(), ids, usernames);
  }
  async addGroupUsers(groupId, ids) {
    return NAKAMA_CLIENT.addGroupUsers(await this.acquireSession(), groupId, ids);
  }
  async banGroupUsers(groupId, ids) {
    return NAKAMA_CLIENT.banGroupUsers(await this.acquireSession(), groupId, ids);
  }
  async blockFriends(ids, usernames) {
    return NAKAMA_CLIENT.blockFriends(await this.acquireSession(), ids, usernames);
  }
  async createGroup(request) {
    return NAKAMA_CLIENT.createGroup(await this.acquireSession(), request);
  }
  createSocket(useSSL, verbose, adapter) {
    return NAKAMA_CLIENT.createSocket(useSSL, verbose, adapter);
  }
  async deleteFriends(ids, usernames) {
    return NAKAMA_CLIENT.deleteFriends(await this.acquireSession(), ids, usernames);
  }
  async deleteGroup(groupId) {
    return NAKAMA_CLIENT.deleteGroup(await this.acquireSession(), groupId);
  }
  async deleteNotifications(ids) {
    return NAKAMA_CLIENT.deleteNotifications(await this.acquireSession(), ids);
  }
  async deleteStorageObjects(request) {
    return NAKAMA_CLIENT.deleteStorageObjects(await this.acquireSession(), request);
  }
  async demoteGroupUsers(groupId, ids) {
    return NAKAMA_CLIENT.demoteGroupUsers(await this.acquireSession(), groupId, ids);
  }
  async emitEvent(request) {
    return NAKAMA_CLIENT.emitEvent(await this.acquireSession(), request);
  }
  async getAccount() {
    return NAKAMA_CLIENT.getAccount(await this.acquireSession());
  }
  async getUsers(ids, usernames, facebookIds) {
    return NAKAMA_CLIENT.getUsers(await this.acquireSession(), ids, usernames, facebookIds);
  }
  async importFacebookFriends(request) {
    return NAKAMA_CLIENT.importFacebookFriends(await this.acquireSession(), request);
  }
  async joinGroup(groupId) {
    return NAKAMA_CLIENT.joinGroup(await this.acquireSession(), groupId);
  }
  async joinTournament(tournamentId) {
    return NAKAMA_CLIENT.joinTournament(await this.acquireSession(), tournamentId);
  }
  async kickGroupUsers(groupId, ids) {
    return NAKAMA_CLIENT.kickGroupUsers(await this.acquireSession(), groupId, ids);
  }
  async leaveGroup(groupId) {
    return NAKAMA_CLIENT.leaveGroup(await this.acquireSession(), groupId);
  }
  async listChannelMessages(channelId, limit, forward, cursor) {
    return NAKAMA_CLIENT.listChannelMessages(await this.acquireSession(), channelId, limit, forward, cursor);
  }
  async listGroupUsers(groupId, state, limit, cursor) {
    return NAKAMA_CLIENT.listGroupUsers(await this.acquireSession(), groupId, state, limit, cursor);
  }
  async listUserGroups(userId, state, limit, cursor) {
    return NAKAMA_CLIENT.listUserGroups(await this.acquireSession(), userId, state, limit, cursor);
  }
  async listGroups(name, cursor, limit) {
    return NAKAMA_CLIENT.listGroups(await this.acquireSession(), name, cursor, limit);
  }
  async linkApple(request) {
    return NAKAMA_CLIENT.linkApple(await this.acquireSession(), request);
  }
  async linkCustom(request) {
    return NAKAMA_CLIENT.linkCustom(await this.acquireSession(), request);
  }
  async linkDevice(request) {
    return NAKAMA_CLIENT.linkDevice(await this.acquireSession(), request);
  }
  async linkEmail(request) {
    return NAKAMA_CLIENT.linkEmail(await this.acquireSession(), request);
  }
  async linkFacebook(request) {
    return NAKAMA_CLIENT.linkFacebook(await this.acquireSession(), request);
  }
  async linkFacebookInstantGame(request) {
    return NAKAMA_CLIENT.linkFacebookInstantGame(await this.acquireSession(), request);
  }
  async linkGameCenter(request) {
    return NAKAMA_CLIENT.linkGameCenter(await this.acquireSession(), request);
  }
  async linkGoogle(request) {
    return NAKAMA_CLIENT.linkGoogle(await this.acquireSession(), request);
  }
  async linkSteam(request) {
    return NAKAMA_CLIENT.linkSteam(await this.acquireSession(), request);
  }
  async listFriends(state, limit, cursor) {
    return NAKAMA_CLIENT.listFriends(await this.acquireSession(), state, limit, cursor);
  }
  async listLeaderboardRecords(leaderboardId, ownerIds, limit, cursor, expiry) {
    return NAKAMA_CLIENT.listLeaderboardRecords(await this.acquireSession(), leaderboardId, ownerIds, limit, cursor, expiry);
  }
  async listLeaderboardRecordsAroundOwner(leaderboardId, ownerId, limit, expiry) {
    return NAKAMA_CLIENT.listLeaderboardRecordsAroundOwner(await this.acquireSession(), leaderboardId, ownerId, limit, expiry);
  }
  async listMatches(limit, authoritative, label, minSize, maxSize, query) {
    return NAKAMA_CLIENT.listMatches(await this.acquireSession(), limit, authoritative, label, minSize, maxSize, query);
  }
  async listNotifications(limit, cacheableCursor) {
    return NAKAMA_CLIENT.listNotifications(await this.acquireSession(), limit, cacheableCursor);
  }
  async listStorageObjects(collection, userId, limit, cursor) {
    return NAKAMA_CLIENT.listStorageObjects(await this.acquireSession(), collection, userId, limit, cursor);
  }
  async listTournaments(categoryStart, categoryEnd, startTime, endTime, limit, cursor) {
    return NAKAMA_CLIENT.listTournaments(await this.acquireSession(), categoryStart, categoryEnd, startTime, endTime, limit, cursor);
  }
  async listTournamentRecords(tournamentId, ownerIds, limit, cursor, expiry) {
    return NAKAMA_CLIENT.listTournamentRecords(await this.acquireSession(), tournamentId, ownerIds, limit, cursor, expiry);
  }
  async listTournamentRecordsAroundOwner(tournamentId, ownerId, limit, expiry) {
    return NAKAMA_CLIENT.listTournamentRecordsAroundOwner(await this.acquireSession(), tournamentId, ownerId, limit, expiry);
  }
  async promoteGroupUsers(groupId, ids) {
    return NAKAMA_CLIENT.promoteGroupUsers(await this.acquireSession(), groupId, ids);
  }
  async readStorageObjects(request) {
    return NAKAMA_CLIENT.readStorageObjects(await this.acquireSession(), request);
  }
  async rpc(id, input) {
    return NAKAMA_CLIENT.rpc(await this.acquireSession(), id, input);
  }
  async rpcGet(id, httpKey, input) {
    return NAKAMA_CLIENT.rpcGet(id, await this.acquireSession(), httpKey, input);
  }
  async unlinkApple(request) {
    return NAKAMA_CLIENT.unlinkApple(await this.acquireSession(), request);
  }
  async unlinkCustom(request) {
    return NAKAMA_CLIENT.unlinkCustom(await this.acquireSession(), request);
  }
  async unlinkDevice(request) {
    return NAKAMA_CLIENT.unlinkDevice(await this.acquireSession(), request);
  }
  async unlinkEmail(request) {
    return NAKAMA_CLIENT.unlinkEmail(await this.acquireSession(), request);
  }
  async unlinkFacebook(request) {
    return NAKAMA_CLIENT.unlinkFacebook(await this.acquireSession(), request);
  }
  async unlinkFacebookInstantGame(request) {
    return NAKAMA_CLIENT.unlinkFacebookInstantGame(await this.acquireSession(), request);
  }
  async unlinkGameCenter(request) {
    return NAKAMA_CLIENT.unlinkGameCenter(await this.acquireSession(), request);
  }
  async unlinkGoogle(request) {
    return NAKAMA_CLIENT.unlinkGoogle(await this.acquireSession(), request);
  }
  async unlinkSteam(request) {
    return NAKAMA_CLIENT.unlinkSteam(await this.acquireSession(), request);
  }
  async updateAccount(request) {
    return NAKAMA_CLIENT.updateAccount(await this.acquireSession(), request);
  }
  async updateGroup(groupId, request) {
    return NAKAMA_CLIENT.updateGroup(await this.acquireSession(), groupId, request);
  }
  async writeLeaderboardRecord(leaderboardId, request) {
    return NAKAMA_CLIENT.writeLeaderboardRecord(await this.acquireSession(), leaderboardId, request);
  }
  async writeStorageObjects(objects) {
    return NAKAMA_CLIENT.writeStorageObjects(await this.acquireSession(), objects);
  }
  async writeTournamentRecord(tournamentId, request) {
    return NAKAMA_CLIENT.writeTournamentRecord(await this.acquireSession(), tournamentId, request);
  }
} // class SessionManager
NakamaSessionManager.instance = new NakamaSessionManager();
const nakama = NakamaSessionManager.instance;
...
const VtAuth = class {
  ...
  async signIn(event) {
    ...
    try {
      ...
      await router.push(`/profile/Profile`, 'forward');
    }
    catch (e) {
  ...
  render() {
    return [
      h("ion-header", { translucent: true }, h("ion-toolbar", null, h("ion-title", null, "Sign In"))),
      h("ion-content", { fullscreen: true }, h("ion-header", { collapse: "condense" }, h("ion-toolbar", null, h("ion-title", { size: "large" }, "Sign In"))), h("form", { onSubmit: (e) => this.signIn(e) }, h("ion-list", { lines: "full", class: "ion-no-margin ion-no-padding" }, h("ion-item", null, h("ion-input", { required: true, type: "email", placeholder: "email", onInput: (e) => this.email = e.target.value, value: this.email })), h("ion-item", null, h("ion-input", { required: true, type: "password", placeholder: "password", onInput: (e) => this.password = e.target.value, value: this.password }))), h("div", { class: "ion-padding" }, h("ion-button", { class: "ion-no-margin", expand: "block", type: "submit" }, "Sign In")))),
    ];
  }
};
VtAuth.style = vtAuthCss;

export { VtAuth as vt_auth };
